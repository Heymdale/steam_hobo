"""
!!!WARNING!!! На данный момент не реализована поддержка линукса, представленный код древний и работать не будет.
В файле sudoers должно быть прописано ALL=NOPASSWD: ALL для главного пользователя,
...
def close_warning():
    ewmh = EWMH()
    warning_windows = ewmh_wrapper(config.linux_warning_headline)
...
def main_os():
    ...
    while True:
        ...
        concurrent_users_count = config.linux_concurrent_users
